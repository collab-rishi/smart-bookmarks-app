...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        // This ensures the user is redirected back to your dashboard after Google login
    ...
    <div className="min-h-screen bg-[#fafafa] flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-sm bg-white border border-gray-100 rounded-2xl shadow-sm p-8 text-center">
        ...
        <p className="text-sm text-gray-500 mb-8">
          A smart manager for your bookmarks.
        </p>
        
        <button
          onClick={handleGoogleLogin}
          className="w-full flex items-center justify-center gap-3 px-4 py-2.5 border border-gray-200 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-all duration-200"
        >
      ...
      <p className="mt-8 text-[10px] text-gray-400 font-bold uppercase tracking-widest">
        Enterprise-Grade Security via Supabase
      </p>
    </div>
  )
}